...
class Metadata:
    ...
    def subject_subject_similarity(self,data_frame1,data_frame2, model, dataset_images_features):
        ...
        subject1_db_matrix = self.reduce_subject_dim(subject1_map)
        subject2_db_matrix = self.reduce_subject_dim(subject2_map)

        for i in range(16):
            similarity_val += self.euclidean_distance(subject1_db_matrix[i], subject2_db_matrix[i])
    ...
    def get_metadata_sift_feature_vector(self, data_frame, metadata, dataset_images_features):
        ...
        #filtered_data_frame = data_frame[(data_frame['aspectOfHand'] == metadata[0]) & (data_frame['accessories'] == metadata[1]) & (data_frame['gender'] == metadata[2])]
        ...
        list = filtered_data_frame['imageName'].tolist()
    ...
    def reduce_subject_dim(self,subject_map):
        database_matrix = []
